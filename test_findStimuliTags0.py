...
for tweet in tweets:
    ...
    if subject_count == 1 and tag_count <= 5:
        ...
        selected_tweets.append(diff(tweet, stimuli))
...
def embed_tweet(tweet):
    index_of_tokens = np.where(np.isin(vocab, tweet))
...
np.savetxt('outputs/subject_classification_dataset_0.txt', v, fmt='%s', delimiter=',')
